...
class AgentChatService:
    ...
    def _looks_like_java_stacktrace(self, text: str) -> bool:
        ...
        lowered = text.lower()
        score = sum(1 for item in markers if item in lowered)
        return score >= 2
    ...
    def _handle_leetcode(self, text: str) -> Dict[str, Any]:
        ...
        return run_leetcode_coach(payload)
    ...
    def _extract_leetcode_payload(self, text: str) -> Dict[str, Any]:
        ...
        problem_statement = non_code_text if non_code_text else "Please analyze the provided LeetCode problem and user code."
        if code and code in problem_statement:
            problem_statement = problem_statement.replace(code, "").strip()
    ...
    def _handle_nl_to_sql(self, text: str) -> Dict[str, Any]:
        ...
        return generate_nl_sql(question=text, account=account)
    ...
    def _format_nl_to_sql_answer(self, data: Dict[str, Any]) -> str:
        ...
        if columns:
            parts.append("结果列\n" + self._numbered(columns))
        tables = data.get("tables") or []
        if tables:
            parts.append("涉及表\n" + self._numbered(tables))
        return "\n\n".join(parts)
