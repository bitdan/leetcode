...
class MarketReviewService:
    ...
    def candidates_2_to_3(
        ...
        for stock in stocks:
            if stock.consecutive_boards != 2:
                continue
            sector = sector_map.get(stock.industry or "未分类")
            score = stock.board_quality_score + ((sector.strength_score if sector else 0) * 0.35)
            reasons = []
