...
3) 禁止注释：不允许出现 “--” 或 “/* */”。
4) 必须使用占位符参数（?），禁止把 account/site 等敏感条件写成常量直接拼进 SQL。
5) 必须包含 deleted 过滤：涉及到的每张表都必须加 deleted = 0。
6) 必须包含 account 过滤：涉及到的每张表都必须加 account = ?。
...
def _parse_account_site(account_token: str) -> Tuple[str, str]:
    ...
    return account.strip(), site.strip().upper()
...
def _validate_sql(sql: str, tables: List[str], require_site: bool) -> Optional[str]:
    ...
    if not re.search(r"\baccount\s*=\s*\?", normalized, re.IGNORECASE):
        return "必须使用 account = ? 过滤"

    if re.search(r"\baccount\s*=\s*'[^']+'", normalized, re.IGNORECASE):
        return "account 不能直接写常量"
    ...
    if has_order and not has_item:
        if not re.search(r"\bo\.account\s*=\s*\?", normalized, re.IGNORECASE):
    ...
    if has_item and not has_order:
        if not re.search(r"\bi\.account\s*=\s*\?", normalized, re.IGNORECASE):
...
def _validate_params(sql: str, params: List[str], require_site: bool) -> Optional[str]:
...
def _build_user_prompt(account_token: str, account: str, site: str, question: str) -> str:
    ...
    return (
        "【上下文】\n"
        f"- 输入 account 参数 = {account_token}\n"
        f"- 解析结果：account = {account}，site = {site_text or '(空)'}\n"
        f"- 必须过滤 account = {account}\n"
        f"- 默认站点 site = {site_text}（为空则不加该过滤）\n"
...
def generate_sql_payload(account_token: str, question: str) -> Dict[str, Any]:
    ...
    if not account_token or not account_token.strip():
        return _empty_result("account 不能为空")
    ...
    err = _validate_sql(result["sql"], result["tables"], require_site=bool(parsed_site))
    if err:
        return _empty_result(f"生成 SQL 未通过安全校验，{err}")

    params_err = _validate_params(result["sql"], result["params"], require_site=bool(parsed_site))
    if params_err:
