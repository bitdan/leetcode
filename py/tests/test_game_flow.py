...
async def test_complete_flow():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print("3. 创建游戏房间...")
        async with session.post(f"{BASE_URL}/api/v1/game/create-room", json={}, headers=headers) as resp:
            if resp.status == 200:
                data = await resp.json()
        ...
        async with session.get(f"{BASE_URL}/api/v1/game/room/{room_id}", headers=headers) as resp:
            if resp.status == 200:
                data = await resp.json()
        ...
        print("6. 离开房间...")
        async with session.post(f"{BASE_URL}/api/v1/game/leave-room", json={}, headers=headers) as resp:
            if resp.status == 200:
                print("离开房间成功")
            else:
                text = await resp.text()
